...
const App = () => {
    ...
    const selectPropertyForEvent = async (deal) => {
        ...
        setEventForm(prev => ({ ...prev, propertyId: deal.id || '', propertyName: `${deal.customer_name||''} ${deal.name||''}`.trim(), name: deal.name || '', category: mappedCategory, customerName: deal.customer_name || '', customerAndpadId: deal.customer_andpad_id ? String(deal.customer_andpad_id) : '', address: address, googleMapUrl: deal.google_map_url || '', systemId: deal.andpad_id ? String(deal.andpad_id) : '', salesRep: deal.role_sales || '', icRep: deal.role_ic || '', constructionRep: deal.role_construction || '', handoverDate: deal.handover_date_actual || deal.handover_date_planned || '' }));
    ...
    const selectPropertyForRequest = async (deal) => {
        ...
        const dealAsProp = {
            id: null, name: deal.name || '', customerName: deal.customer_name || '', customerAndpadId: deal.customer_andpad_id ? String(deal.customer_andpad_id) : '', address,
