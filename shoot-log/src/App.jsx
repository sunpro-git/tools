...
const App = () => {
    ...
    const [form, setForm] = useState({
        name: '', address: '', category: '新築', subCategory: '', furnitureSetup: 'なし', customerName: '', customerAndpadId: '', event_type: '', customerLat: '', customerLon: '', mainStore: '',
    ...
    const [requestSearched, setRequestSearched] = useState(false);
    const runCustomerSearch = async () => {
        const hasFilter = reqSearch.keyword || reqSearch.staff;
        if (!hasFilter) { setRequestResults([]); setRequestSearched(false); return; }
        ...
        if (reqSearch.staff) {
            query = query.ilike('staff_name', `%${reqSearch.staff}%`);
        }
    ...
    // 顧客の案件一覧を取得（撮影依頼/イベントモーダルでcustomerAndpadIdが変わったら）
    const customerAndpadIdForDeals = (isModalOpen ? form.customerAndpadId : '') || (isEventModalOpen ? eventForm.customerAndpadId : '');
    useEffect(() => {
        if (!customerAndpadIdForDeals) { setCustomerDeals([]); return; }
        let cancelled = false;
        (async () => {
            const { data, error } = await supabase.from('deals')
                .select('id,andpad_id,name,deal_category,role_sales,role_ic,role_construction,order_date,handover_date_actual,handover_date_planned,order_amount,label_office,construction_location')
                .eq('customer_andpad_id', String(customerAndpadIdForDeals))
                .order('order_date', { ascending: false })
                .limit(20);
            if (cancelled) return;
            if (error) { console.error('customer deals fetch error:', error); setCustomerDeals([]); return; }
            setCustomerDeals(data || []);
        })();
        return () => { cancelled = true; };
    }, [customerAndpadIdForDeals]);
    ...
    // 開発環境(import.meta.env.DEV)はログインを省略してダミーユーザーで起動
    useEffect(() => {
        if (import.meta.env.DEV) {
            setUser({ id: 'dev-user', email: `dev@${ALLOWED_DOMAIN}`, user_metadata: { full_name: '開発ユーザー' } });
    ...
    const listenToData = () => {
        ...
        const fetchSettings = async () => {
            ...
            const s = {}; const et = {};
            (data || []).forEach(r => {
                if (r.key.startsWith('chatwork_')) s[r.key] = r.value || '';
                if (r.key.startsWith('event_type_')) et[r.key] = r.value || '';
            });
            setChatworkSettings(prev => ({ ...prev, ...s }));
            if (Object.keys(et).length > 0) setEventTypeSettings(prev => ({ ...prev, ...et }));
        };
    ...
    const renderCwTemplate = (template, prop, extraCtx = {}) => {
        ...
        const map = {
            '{案件名}': (prop.customerName || prop.eventName || '').replace(/[\s\u3000]+/g, ''), '{住所}': prop.address || '', '{種別}': prop.category || '',
    ...
    const saveEventTypeSettings = async () => {
        const upserts = Object.entries(eventTypeSettings).map(([key, value]) => ({ key, value: String(value || ''), updated_at: new Date().toISOString() }));
        const { error } = await supabase.from('app_settings').upsert(upserts);
        if (error) { alert('設定保存失敗: ' + error.message); return; }
        showNotification('イベント種別設定を保存しました');
        setIsEventTypeSettingsOpen(false);
    };
    ...
    const openEditModal = (prop) => {
        ...
        setForm({
            ...prop, category: cat,
            customerName: prop.customerName||'', customerAndpadId: prop.customerAndpadId||'', event_type: prop.event_type||'', customerLat: prop.customerLat||'', customerLon: prop.customerLon||'', mainStore: prop.mainStore||'', googleMapUrl: prop.googleMapUrl||'', handoverSource: prop.handoverSource||'', furnitureSetup: prop.furnitureSetup||'なし',
    ...
    const handleCategoryChange = (cat) => { let newTypes = []; if (cat === '新築') newTypes = ['スチール', 'YouTube', 'インスタライブ']; else if (cat === 'リフォーム') newTypes = ['スチール', 'YouTube', 'インスタ通常投稿用', 'インスタ宣伝用']; setForm(prev => ({ ...prev, category: cat, subCategory: cat === '新築' ? prev.subCategory : '', shootingTypes: newTypes })); };
    ...
    const saveEvent = async (e) => {
        ...
        // event_type: 手動入力があればそれを使用、なければ登録内容から自動判定
        data.event_type = computeEventType({...data}, eventTypeSettings.event_type_ohirome_pattern) || 'event';
        try {
    ...
    const openRequestModal = () => { setRequestMode('shoot'); setIsRequestModalOpen(true); setReqSearch({keyword:'', category:'新築', staff:'', contractFrom:'2020-01', contractTo:'', handoverFrom:'', handoverTo:''}); setRequestResults([]); setRequestSearched(false); };
...
A. ヘッダーのチャットワーク設定アイコンからルームIDと通知タイミングを設定してください。`;
    ...
    return (
        <div className="min-h-screen pb-20">
                ...
                <div className="bg-white rounded-2xl shadow-card border border-gray-100 p-1 mb-8">
                    <div className="flex flex-col lg:flex-row items-center justify-between gap-4 p-4">
                        <div className="flex flex-col sm:flex-row items-center gap-4 w-full lg:w-auto">
                            <div className="flex items-center gap-2 bg-gray-200 rounded-xl px-3 py-2 border border-gray-300"><Icon name="calendar-days" size={18} className="text-gray-500" /><input type="month" value={filterFrom} onChange={e => setFilterFrom(e.target.value)} className="bg-transparent text-sm font-semibold outline-none text-gray-700 w-28" /><span className="text-gray-300">→</span><input type="month" value={filterTo} onChange={e => setFilterTo(e.target.value)} className="bg-transparent text-sm font-semibold outline-none text-gray-700 w-28" /></div>
                                        ...
                                        <React.Fragment key={group.title}>
                                            ...
                                            {group.items.map(item => {
                                                if (item._type === 'event') {
                                                    ...
                                                    return (
                                                        <tr key={`evt-${evt.id}`} onDoubleClick={() => openEditModal(evt)} className="hover:bg-teal-50 transition-colors border-b-2 border-gray-300 bg-teal-50/30 cursor-pointer">
                                                            <td className="px-2 py-3 align-top">{(() => { const evtBranch = getAreaBranchDynamic(evt.address, evt.category); const _ed = (evt.eventDates && evt.eventDates.filter(Boolean).length > 0 ? evt.eventDates : (evt.dates || [])).filter(Boolean).map(d => (typeof d === 'string' ? d.split('T')[0] : (d instanceof Date ? d.toISOString().split('T')[0] : String(d).split('T')[0]))); const _today = new Date().toISOString().split('T')[0]; const _statusBadge = _ed.length === 0 ? <span className="px-1.5 py-0.5 rounded text-xs font-bold bg-teal-100 text-teal-700 border border-teal-200">イベント</span> : (() => { const sorted = [..._ed].sort(); const first = sorted[0]; const last = sorted[sorted.length-1]; if (_today > last) return <span className="px-1.5 py-0.5 rounded text-xs font-bold bg-gray-100 text-gray-500 border border-gray-200">イベント終了</span>; if (_today >= first) return <span className="px-1.5 py-0.5 rounded text-xs font-bold bg-emerald-100 text-emerald-700 border border-emerald-200">イベント開催中</span>; return <span className="px-1.5 py-0.5 rounded text-xs font-bold bg-blue-100 text-blue-700 border border-blue-200">イベント予定</span>; })(); return (<div className="flex flex-col gap-1 items-start">{_statusBadge}{(() => { const _ss = computeShootStatus(evt); return _ss ? <span className={`px-1.5 py-0.5 rounded text-xs font-bold border ${_ss.cls}`}>{_ss.label}</span> : null; })()}</div>); })()}</td>
                                                            <td className="px-2 py-3 align-top">
                                                                {(() => { const hasCustomer = evt.customerName && evt.customerName !== '顧客なし'; const fallbackTitle = evt.eventName || (evt.name && evt.name !== '案件なし' ? evt.name : ''); return (<div className="mb-1"><div className="font-bold text-sm text-gray-900">{hasCustomer ? <>{evt.customerName}<span className="text-xs font-normal text-gray-600">様</span></> : (fallbackTitle || <span className="text-gray-400 font-normal">イベント名未設定</span>)}</div>{hasCustomer && fallbackTitle && <div className="text-xs text-gray-500 mt-0.5">{fallbackTitle}</div>}<div className="flex items-center gap-1.5 flex-wrap mt-1">{evt.category && <span className="px-1.5 py-0.5 rounded text-[10px] font-bold bg-gray-100 text-gray-600 border border-gray-200">{evt.category}</span>}{(() => { const evtBranch = getAreaBranchDynamic(evt.address, evt.category); return evtBranch && <span className={`px-1.5 py-0.5 rounded text-[10px] font-bold border ${evtBranch.class}`}>{evtBranch.name}</span>; })()}{evt.event_type && <span className="px-1.5 py-0.5 rounded text-[10px] font-bold bg-purple-100 text-purple-700 border border-purple-200">{(EVENT_TYPE_OPTIONS.find(o=>o.value===evt.event_type)||{label:evt.event_type}).label}</span>}</div></div>); })()}
                                                                {evt.address && <div className="text-xs text-gray-500 flex items-center gap-1 mb-1"><Icon name="map-pin" size={12}/> {evt.address}</div>}
                                                            ...
                                                            <td className="px-2 py-3 align-top">
                                                                <div className="space-y-1">
                                                                    {evt.shootingRangeFrom && <><div className="flex items-center gap-2 text-sm whitespace-nowrap"><span className="w-20 font-bold text-amber-600 text-sm">撮影可能</span><span className="font-normal text-gray-700">{formatDateTime(evt.shootingRangeFrom)}</span></div><div className="flex items-center gap-2 text-sm whitespace-nowrap"><span className="w-20"></span><span className="font-normal text-gray-700"><span className="text-[10px] text-gray-400">→ </span>{evt.shootingRangeTo ? formatDateTime(evt.shootingRangeTo) : <span className="text-gray-400">未設定</span>}</span></div></>}
                                                ...
                                                return (
                                                    <tr key={prop.id} onDoubleClick={() => openEditModal(prop)} className="hover:bg-yellow-50 transition-colors border-b-2 border-gray-300 cursor-pointer">
                                                        <td className="px-2 py-3 align-top"><div className="flex flex-col gap-1 items-start">{(() => { const _ss = computeShootStatus(prop); return _ss ? <span className={`px-1.5 py-0.5 rounded text-xs font-bold border ${_ss.cls}`}>{_ss.label}</span> : null; })()}</div></td>
                                                        <td className="px-2 py-3 align-top">
                                                            {(() => { const hasCustomer = prop.customerName && prop.customerName !== '顧客なし'; const fallbackTitle = prop.eventName || (prop.name && prop.name !== '案件なし' ? prop.name : ''); return (<div className="mb-1"><div className="font-bold text-sm text-gray-900">{hasCustomer ? <>{prop.customerName}<span className="text-xs font-normal text-gray-600">様</span></> : (fallbackTitle || <span className="text-gray-400 font-normal">顧客名未設定</span>)}</div>{hasCustomer && fallbackTitle && <div className="text-xs text-gray-500 mt-0.5">{fallbackTitle}</div>}<div className="flex items-center gap-1.5 flex-wrap mt-1">{prop.category && <span className="px-1.5 py-0.5 rounded text-[10px] font-bold bg-gray-100 text-gray-600 border border-gray-200">{prop.category}</span>}{branch && <span className={`px-1.5 py-0.5 rounded text-[10px] font-bold border ${branch.class}`}>{branch.name}</span>}{prop.event_type && <span className="px-1.5 py-0.5 rounded text-[10px] font-bold bg-purple-100 text-purple-700 border border-purple-200">{(EVENT_TYPE_OPTIONS.find(o=>o.value===prop.event_type)||{label:prop.event_type}).label}</span>}</div></div>); })()}
                                                            <div className="text-xs text-gray-500 flex items-center gap-1 mb-1"><Icon name="map-pin" size={12}/> {prop.address}</div>
                                                        ...
                                                        <td className="px-2 py-3 align-top">
                                                            <div className="space-y-1">
                                                                {prop.shootingRangeFrom && <><div className="flex items-center gap-2 text-sm whitespace-nowrap"><span className="w-20 font-bold text-amber-600 text-sm">撮影可能</span><span className="font-normal text-gray-700">{formatDateTime(prop.shootingRangeFrom)}</span></div><div className="flex items-center gap-2 text-sm whitespace-nowrap"><span className="w-20"></span><span className="font-normal text-gray-700"><span className="text-[10px] text-gray-400">→ </span>{prop.shootingRangeTo ? formatDateTime(prop.shootingRangeTo) : <span className="text-gray-400">未設定</span>}</span></div></>}
            ...
            {/* Modal: Property Edit */}
            {isModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4"><div className="absolute inset-0 bg-primary/40 backdrop-blur-sm" onClick={() => setIsModalOpen(false)}></div><div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl max-h-[90vh] overflow-y-auto relative animate-enter">
                    <div className="sticky top-0 bg-white/95 backdrop-blur z-20 px-8 py-5 border-b border-gray-100 flex justify-between items-center">
                        <div className="flex flex-col"><h2 className="text-xs font-bold text-gray-400 uppercase tracking-widest">{editingId ? '撮影依頼' : '撮影依頼'}</h2><div className="text-xl font-bold text-primary">{form.customerName || '顧客名未設定'}</div></div>
                        <div className="flex items-center gap-3">{form.systemId && <a href={`https://andpad.jp/manager/my/orders/${form.systemId}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 px-3 py-1.5 text-red-600 bg-red-50 hover:bg-red-100 border border-red-200 rounded-lg transition-colors font-bold text-xs" title="ANDPAD">ANDPAD <Icon name="external-link" size={11}/></a>}<button onClick={() => setIsModalOpen(false)} className="p-2 hover:bg-gray-100 rounded-full transition-colors ml-2"><Icon name="x" size={24}/></button></div>
                    </div>
                    <div className="p-8 space-y-8"><form onSubmit={saveProperty} className="space-y-8">
                        <section className="grid grid-cols-1 md:grid-cols-[120px_1fr] gap-6"><h3 className="text-sm font-bold text-gray-400 uppercase tracking-widest pt-3">基本情報</h3><div className="space-y-4"><div className="flex gap-4 items-start"><div className="flex-1"><label className="block text-sm font-bold text-gray-500 mb-1">顧客名</label><div className="flex gap-2"><input type="text" value={form.customerName} onChange={e => setForm({...form, customerName: e.target.value})} disabled={editingId && form.customerName === '顧客なし'} className={`flex-1 px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-accent ${editingId && form.customerName === '顧客なし' ? 'opacity-50 cursor-not-allowed' : ''}`} /><button type="button" onClick={()=>openCustomerPickerForPatch('patch_property')} className="px-3 py-2.5 text-blue-600 bg-blue-50 hover:bg-blue-100 border border-blue-200 rounded-xl transition-colors font-bold text-xs whitespace-nowrap flex items-center gap-1" title="顧客を選び直す"><Icon name="search" size={14}/>顧客選択</button></div></div>{form.customerAndpadId && <div><label className="block text-sm font-bold text-gray-500 mb-1">顧客ID</label><div className="flex items-center gap-2 py-2.5"><span className="text-sm font-mono text-gray-700 whitespace-nowrap">{form.customerAndpadId}</span><a href={`https://andpad.jp/manager/my/customers/${form.customerAndpadId}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 px-3 py-1.5 text-red-600 bg-red-50 hover:bg-red-100 border border-red-200 rounded-lg transition-colors font-bold text-xs whitespace-nowrap" title="ANDPAD顧客ページを開く">ANDPAD <Icon name="external-link" size={11}/></a></div></div>}</div>{form.customerAndpadId && customerDeals.length > 0 && (<div><label className="block text-sm font-bold text-gray-500 mb-1">この顧客の案件 ({customerDeals.length}件)</label><div className="border border-gray-200 rounded-xl divide-y divide-gray-100 max-h-56 overflow-y-auto bg-white">{customerDeals.map(d => (<a key={d.id} href={`https://andpad.jp/manager/my/orders/${d.andpad_id}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 px-3 py-2 hover:bg-gray-50 transition-colors text-xs"><span className="px-1.5 py-0.5 rounded text-[10px] font-bold bg-gray-100 text-gray-600 border border-gray-200 whitespace-nowrap">{d.deal_category || '-'}</span><span className="flex-1 truncate font-medium text-gray-800">{d.name || '案件名なし'}</span>{d.order_date && <span className="text-gray-400 whitespace-nowrap">契約 {formatDate(d.order_date)}</span>}{(d.handover_date_actual || d.handover_date_planned) && <span className="text-emerald-600 whitespace-nowrap">引渡 {formatDate(d.handover_date_actual || d.handover_date_planned)}</span>}<Icon name="external-link" size={12} className="text-gray-400 flex-shrink-0"/></a>))}</div></div>)}<div className="grid grid-cols-2 gap-4"><div><label className="block text-sm font-bold text-gray-500 mb-1">住所</label><input type="text" value={form.address} onChange={e => setForm({...form, address: e.target.value})} className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-accent" /></div><div><label className="block text-sm font-bold text-gray-500 mb-1">GoogleMapURL</label><div className="flex gap-2"><input type="text" value={form.googleMapUrl} onChange={e => setForm({...form, googleMapUrl: e.target.value})} className="flex-1 px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-accent" />{form.googleMapUrl && <a href={form.googleMapUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 px-3 py-2.5 text-blue-600 bg-blue-50 hover:bg-blue-100 border border-blue-200 rounded-xl transition-colors font-bold text-xs whitespace-nowrap" title="Google Map">MAP <Icon name="external-link" size={11}/></a>}</div></div></div><div className="grid grid-cols-2 gap-4"><div><label className="block text-sm font-bold text-gray-500 mb-2">事業部</label><div className="flex flex-wrap gap-2">{CATEGORIES.map(c=><button key={c} type="button" onClick={()=>handleCategoryChange(c)} className={`px-4 py-2 rounded-lg text-sm font-bold border ${form.category===c?'bg-accent text-white':'bg-white text-gray-500'}`}>{c}</button>)}</div></div>{form.category==='新築' && <div><label className="block text-sm font-bold text-gray-500 mb-2">ブランド</label><div className="flex flex-wrap gap-2">{['建築設計','LIFIT'].map(s=><button key={s} type="button" onClick={()=>setForm({...form,subCategory:form.subCategory===s?'':s})} className={`px-4 py-2 rounded-lg text-sm font-bold border ${form.subCategory===s?'bg-accent text-white':'bg-white text-gray-500'}`}>{s}</button>)}</div></div>}</div></div></section>
                        <section className="grid grid-cols-1 md:grid-cols-[120px_1fr] gap-6 border-t border-gray-100 pt-8"><h3 className="text-sm font-bold text-gray-400 uppercase tracking-widest pt-3">スケジュール</h3><div className="space-y-6"><div><label className="text-sm font-bold text-blue-600 mb-1 block">設営</label><div className="flex gap-2 items-center flex-wrap"><span className="relative inline-block"><input type="date" value={form.setupDate_date} onChange={e=>setForm({...form,setupDate_date:e.target.value})} onClick={handleShowPicker} className="w-36 px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-accent text-sm font-bold pr-5"/><DowLabel ymd={form.setupDate_date} className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none bg-gray-50 px-0.5"/></span><select value={form.setupDate_time} onChange={e=>setForm({...form,setupDate_time:e.target.value})} className="w-[5.5rem] px-2 py-2.5 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-accent text-sm font-bold"><option value="">開始</option>{timeOptions.map(t=><option key={t} value={t}>{t}</option>)}</select><span className="text-gray-400">→</span><select value={form.setupEndTime} onChange={e=>setForm({...form,setupEndTime:e.target.value})} className="w-[5.5rem] px-2 py-2.5 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-accent text-sm font-bold"><option value="">終了</option>{timeOptions.map(t=><option key={t} value={t}>{t}</option>)}</select><select value={form.setupVehicle} onChange={e=>setForm({...form,setupVehicle:e.target.value})} className="w-[13.3rem] px-2 py-2.5 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-accent text-sm font-bold"><option value="">車両1</option>{vehicleOptions.map(v=><option key={v.id} value={v.name}>{v.name}</option>)}</select><select value={form.setupVehicle2} onChange={e=>setForm({...form,setupVehicle2:e.target.value})} className="w-[13.3rem] px-2 py-2.5 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-accent text-sm font-bold"><option value="">車両2</option>{vehicleOptions.map(v=><option key={v.id} value={v.name}>{v.name}</option>)}</select></div></div><div><label className="text-sm font-bold text-orange-600 mb-1 block">撤収</label><div className="flex gap-2 items-center flex-wrap"><span className="relative inline-block"><input type="date" value={form.teardownDate_date} onChange={e=>setForm({...form,teardownDate_date:e.target.value})} onClick={handleShowPicker} className="w-36 px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-accent text-sm font-bold pr-5"/><DowLabel ymd={form.teardownDate_date} className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none bg-gray-50 px-0.5"/></span><select value={form.teardownDate_time} onChange={e=>setForm({...form,teardownDate_time:e.target.value})} className="w-[5.5rem] px-2 py-2.5 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-accent text-sm font-bold"><option value="">開始</option>{timeOptions.map(t=><option key={t} value={t}>{t}</option>)}</select><span className="text-gray-400">→</span><select value={form.teardownEndTime} onChange={e=>setForm({...form,teardownEndTime:e.target.value})} className="w-[5.5rem] px-2 py-2.5 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-accent text-sm font-bold"><option value="">終了</option>{timeOptions.map(t=><option key={t} value={t}>{t}</option>)}</select><select value={form.teardownVehicle} onChange={e=>setForm({...form,teardownVehicle:e.target.value})} className="w-[13.3rem] px-2 py-2.5 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-accent text-sm font-bold"><option value="">車両1</option>{vehicleOptions.map(v=><option key={v.id} value={v.name}>{v.name}</option>)}</select><select value={form.teardownVehicle2} onChange={e=>setForm({...form,teardownVehicle2:e.target.value})} className="w-[13.3rem] px-2 py-2.5 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-accent text-sm font-bold"><option value="">車両2</option>{vehicleOptions.map(v=><option key={v.id} value={v.name}>{v.name}</option>)}</select></div></div><div className="border-t border-dashed pt-4"><div className="mb-4"><label className="text-sm font-bold text-purple-600 mb-1 block">イベント名</label><input type="text" value={form.eventName} onChange={e=>setForm({...form,eventName:e.target.value})} className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-accent text-sm font-bold" placeholder="例: 完成見学会"/></div><div className="mb-4"><label className="text-sm font-bold text-purple-600 mb-1 block">イベント日</label>{(form.eventDates||[]).map((d,i)=>(<div key={i} className="flex gap-2 mt-1"><span className="relative inline-block"><input type="date" value={d} onChange={e=>updateFormEventDate(i,e.target.value)} onClick={handleShowPicker} className="w-36 px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-accent text-sm font-bold pr-5"/><DowLabel ymd={d} className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none bg-gray-50 px-0.5"/></span>{(form.eventDates||[]).length>1&&<button type="button" onClick={()=>removeFormEventDate(i)} className="text-red-500"><Icon name="trash-2" size={18}/></button>}</div>))}<button type="button" onClick={addFormEventDate} className="text-sm text-purple-500 mt-1">+ 追加</button></div></div><div><label className="text-sm font-bold text-emerald-600 mb-1 block">引渡日</label><span className="relative inline-block"><input type="date" value={form.handoverDate} onChange={e=>setForm({...form,handoverDate:e.target.value})} onClick={handleShowPicker} className="w-36 px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-accent text-sm font-bold pr-5"/><DowLabel ymd={form.handoverDate} className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none bg-gray-50 px-0.5"/></span></div></div></section>
                            <section className="grid grid-cols-1 md:grid-cols-[120px_1fr] gap-6 border-t border-gray-100 pt-8"><h3 className="text-sm font-bold text-gray-400 uppercase tracking-widest pt-3">撮影依頼</h3><div className="space-y-6"><div><label className="text-sm font-bold text-gray-500 mb-1 block">依頼者</label><select value={form.requester} onChange={e=>setForm({...form,requester:e.target.value})} className="w-36 px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-accent text-sm font-bold"><option value="">選択</option>{staffSuggestions.map(s=><option key={s} value={s}>{s}</option>)}</select></div><div className="flex gap-6 items-start flex-wrap"><div><label className="text-sm font-bold text-accent mb-1 block">撮影可能期間</label><div className="flex gap-2 items-center"><span className="relative inline-block"><input type="date" value={form.shootingRange_from_date} onChange={e=>setForm({...form,shootingRange_from_date:e.target.value})} onClick={handleShowPicker} className="w-36 px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm font-bold pr-5"/><DowLabel ymd={form.shootingRange_from_date} className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none bg-gray-50 px-0.5"/></span><select value={form.shootingRange_from_time} onChange={e=>setForm({...form,shootingRange_from_time:e.target.value})} className="px-2 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm font-bold"><option value="">時間</option>{timeOptions.map(t=><option key={t} value={t}>{t}</option>)}</select><span>~</span><span className="relative inline-block"><input type="date" value={form.shootingRange_to_date} onChange={e=>setForm({...form,shootingRange_to_date:e.target.value})} onClick={handleShowPicker} className="w-36 px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm font-bold pr-5"/><DowLabel ymd={form.shootingRange_to_date} className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none bg-gray-50 px-0.5"/></span><select value={form.shootingRange_to_time} onChange={e=>setForm({...form,shootingRange_to_time:e.target.value})} className="px-2 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm font-bold"><option value="">時間</option>{timeOptions.map(t=><option key={t} value={t}>{t}</option>)}</select></div></div><div className="ml-auto"><label className="block text-sm font-bold text-gray-500 mb-2">家具設営</label><div className="flex flex-wrap gap-2">{['あり','なし'].map(o=><button key={o} type="button" onClick={()=>setForm({...form,furnitureSetup:o})} className={`px-4 py-2 rounded-lg text-sm font-bold border ${form.furnitureSetup===o?'bg-accent text-white':'bg-white text-gray-500'}`}>{o}</button>)}</div></div></div><div><label className="text-sm font-bold text-gray-500 mb-1 block">撮影種類</label><div className="flex flex-wrap gap-2">{SHOOTING_TYPES.map(t=><button type="button" key={t} onClick={()=>{const n=form.shootingTypes.includes(t)?form.shootingTypes.filter(x=>x!==t):[...form.shootingTypes,t];setForm({...form,shootingTypes:n})}} className={`px-3 py-2 border rounded-lg ${form.shootingTypes.includes(t)?'bg-accent text-white':'bg-gray-50'}`}>{t}</button>)}</div></div><div className="grid grid-cols-1 md:grid-cols-2 gap-6"><FileUpload label="撮影指示書 (PDF)" fileUrl={form.instructionFileUrl} fileName={form.instructionFileName} onFileChange={(url, name) => setForm({...form, instructionFileUrl: url, instructionFileName: name})} /><FileUpload label="イベント物件概要書 (PDF)" fileUrl={form.overviewFileUrl} fileName={form.overviewFileName} onFileChange={(url, name) => setForm({...form, overviewFileUrl: url, overviewFileName: name})} /></div><div className="grid grid-cols-2 gap-4"><div><label className="text-sm font-bold text-gray-500 mb-1 block">立ち合い</label><select value={form.witnessStaff} onChange={e=>setForm({...form,witnessStaff:e.target.value})} className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-accent text-sm font-bold"><option value="">選択</option><option value="なし">なし</option>{staffSuggestions.map(s=><option key={s} value={s}>{s}</option>)}</select></div><div><label className="text-sm font-bold text-gray-500 mb-1 block">施主在宅</label><div className="flex gap-2">{['あり','なし'].map(o=><button key={o} type="button" onClick={()=>setForm({...form,ownerPresence:o})} className={`flex-1 py-2.5 rounded-xl border text-sm font-bold transition-all ${form.ownerPresence===o?'bg-accent text-white border-accent':'bg-white text-gray-500 border-gray-200'}`}>{o}</button>)}</div></div></div><div><label className="text-sm font-bold text-gray-500 mb-1 block">撮影の注意事項など</label><textarea value={form.shootingNotes} onChange={e=>setForm({...form,shootingNotes:e.target.value})} rows={3} className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-accent text-sm" placeholder="撮影時の注意点やメモを入力"/></div></div></section>
            ...
            {isRequestModalOpen && (<div className="fixed inset-0 z-50 flex items-center justify-center p-4"><div className="absolute inset-0 bg-primary/60 backdrop-blur-sm" onClick={()=>setIsRequestModalOpen(false)}></div><div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl max-h-[90vh] overflow-y-auto relative animate-enter flex flex-col"><div className="p-6 border-b border-gray-100 bg-white sticky top-0 z-10"><div className="flex justify-between items-center mb-6"><h2 className="text-xl font-bold text-primary flex items-center gap-2"><Icon name={requestMode==='event'?'calendar-plus':'camera'} size={24}/> {requestMode==='event'?'イベント追加':requestMode==='shoot'?'撮影依頼':'顧客の選び直し'} - 顧客選択</h2><div className="flex items-center gap-2">{requestMode==='event' && <button type="button" onClick={()=>{setIsRequestModalOpen(false);setEditingEventId(null);resetEventForm();setEventForm(prev=>({...prev, customerName:'顧客なし', name:'案件なし'}));setIsEventModalOpen(true);}} className="px-4 py-2 bg-gray-100 text-gray-700 rounded-xl text-sm font-bold hover:bg-gray-200 transition-colors whitespace-nowrap flex items-center gap-1"><Icon name="calendar-plus" size={16} className="text-purple-500"/> 顧客選択なしでイベント追加</button>}<button onClick={()=>setIsRequestModalOpen(false)} className="p-2 hover:bg-gray-100 rounded-full transition-colors"><Icon name="x" size={24}/></button></div></div><div className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-gray-50 p-4 rounded-xl border border-gray-200"><div className="col-span-1"><label className="text-xs font-bold text-gray-400 uppercase block mb-1">担当者</label><select value={reqSearch.staff} onChange={e=>setReqSearch({...reqSearch, staff:e.target.value})} className="w-full border rounded-lg px-3 py-2 text-sm bg-white"><option value="">全て</option>{staffSuggestions.map(s=><option key={s} value={s}>{s}</option>)}</select></div><div className="col-span-2"><label className="text-xs font-bold text-gray-400 uppercase block mb-1">フリーワード</label><div className="flex gap-2"><input type="text" value={reqSearch.keyword} onChange={e=>setReqSearch({...reqSearch, keyword:e.target.value})} onKeyDown={e=>{ if (e.key === 'Enter') { e.preventDefault(); runCustomerSearch(); } }} className="flex-1 border rounded-lg px-3 py-2 text-sm" placeholder="顧客名, カナ, 住所, ANDPAD ID..." /><button type="button" onClick={runCustomerSearch} disabled={requestLoading || (!reqSearch.keyword && !reqSearch.staff)} className="px-4 py-2 bg-accent text-white rounded-lg text-sm font-bold hover:bg-accent/90 disabled:bg-gray-300 disabled:cursor-not-allowed transition-colors flex items-center gap-1.5 whitespace-nowrap"><Icon name="search" size={14}/>{requestLoading ? '検索中…' : '検索'}</button></div></div><div className="col-span-3 flex justify-end pt-2"><button onClick={()=>setReqSearch({keyword:'',category:'新築',staff:'',contractFrom:'2020-01',contractTo:'',handoverFrom:'',handoverTo:''})} className="text-sm text-gray-400 underline hover:text-gray-600">条件クリア</button></div></div></div><div className="flex-1 overflow-y-auto p-6 bg-gray-50"><div className="space-y-3"><div className="grid grid-cols-12 gap-4 px-4 py-2 text-xs font-bold text-gray-400 uppercase tracking-wider border-b border-gray-200"><div className="col-span-2">ANDPAD ID</div><div className="col-span-3">顧客名</div><div className="col-span-5">住所</div><div className="col-span-2">担当者</div></div>{requestLoading ? (<div className="text-center py-12 text-gray-400 font-bold flex items-center justify-center gap-2"><svg className="animate-spin h-5 w-5 text-accent" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v4a4 4 0 00-4 4H4z"></path></svg>検索中…</div>) : requestSuggestions.length > 0 ? requestSuggestions.map(c => (<div key={c.id} onClick={() => requestMode==='event' ? selectCustomerForEvent(c) : (requestMode==='shoot' ? selectCustomerForRequest(c) : selectCustomerForPatch(c))} className="grid grid-cols-12 gap-4 px-4 py-4 bg-white rounded-xl border border-gray-200 hover:border-accent hover:shadow-md transition-all cursor-pointer items-center group"><div className="col-span-2 text-xs text-gray-500 font-mono">{c.andpad_id || '-'}</div><div className="col-span-3"><div className="font-bold text-sm text-gray-800">{c.name || '未設定'}</div>{c.name_kana && <div className="text-xs text-gray-400 truncate">{c.name_kana}</div>}</div><div className="col-span-5 text-xs text-gray-600 truncate">{[c.prefecture, c.address].filter(Boolean).join('') || '-'}</div><div className="col-span-2 text-xs text-gray-500">{c.staff_name ? formatRepName(c.staff_name) : '-'}</div></div>)) : (<div className="text-center py-12 text-gray-400 font-bold">{requestSearched ? '条件に一致する顧客がありません' : 'キーワードを入力して「検索」ボタンを押してください'}</div>)}</div></div></div></div>)}
            ...
            {isEventTypeSettingsOpen && (<div className="fixed inset-0 z-50 flex items-center justify-center p-4"><div className="absolute inset-0 bg-primary/60 backdrop-blur-sm" onClick={()=>setIsEventTypeSettingsOpen(false)}></div><div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto animate-enter"><div className="flex justify-between items-center mb-6"><h3 className="text-xl font-bold text-primary flex items-center gap-2"><Icon name="tag" size={24}/> イベント種別設定</h3><button onClick={()=>setIsEventTypeSettingsOpen(false)} className="p-2 hover:bg-gray-100 rounded-full"><Icon name="x" size={20}/></button></div><div className="space-y-5">
                <div className="bg-gray-50 rounded-xl p-4 border border-gray-200">
                    <h4 className="font-bold text-sm text-gray-700 mb-2">自動判定ルール</h4>
                    <p className="text-xs text-gray-500 mb-3">登録時にイベント名/案件名のテキストにマッチした場合、家具設営の有無で <code className="bg-gray-200 px-1 rounded">ohirome</code> / <code className="bg-gray-200 px-1 rounded">ohirome_nashi</code> として保存されます（正規表現可）。</p>
                    <label className="block text-xs font-bold text-gray-500 mb-1">お披露目会・見学会のキーワード（正規表現）</label>
                    <input type="text" value={eventTypeSettings.event_type_ohirome_pattern || ''} onChange={e=>setEventTypeSettings(p=>({...p, event_type_ohirome_pattern: e.target.value}))} className="w-full border rounded-lg px-3 py-2 text-sm font-mono" placeholder="お披露目会|見学会" />
                </div>
                <div className="bg-gray-50 rounded-xl p-4 border border-gray-200">
                    <h4 className="font-bold text-sm text-gray-700 mb-3">eventType の値（GoogleCalendar extendedProperties.shared.eventType に反映）</h4>
                    <div className="space-y-2">
                        {EVENT_TYPE_OPTIONS.map(o => (
                            <div key={o.value} className="flex items-center gap-3 px-3 py-2 bg-white rounded-lg border">
                                <code className="text-xs font-mono bg-gray-100 px-2 py-1 rounded text-gray-700 min-w-[120px]">{o.value}</code>
                                <span className="text-sm text-gray-600">{o.label}</span>
                            </div>
                        ))}
                    </div>
                    <p className="text-xs text-gray-400 mt-3">判定ロジック:</p>
                    <ul className="text-xs text-gray-500 list-disc ml-5 space-y-0.5 mt-1">
                        <li>名称にキーワード一致 + 家具設営=あり → <code>ohirome</code></li>
                        <li>名称にキーワード一致 + 家具設営=なし → <code>ohirome_nashi</code></li>
                        <li>イベント日が登録 → <code>event</code></li>
                        <li>撮影日のみ + 家具設営=あり → <code>satsuei</code></li>
                        <li>撮影日のみ + 家具設営=なし → <code>satsuei_nashi</code></li>
                    </ul>
                </div>
                <div className="flex gap-3 pt-2"><button onClick={()=>setIsEventTypeSettingsOpen(false)} className="flex-1 py-2 border rounded text-gray-500">キャンセル</button><button onClick={saveEventTypeSettings} className="flex-1 py-2 bg-primary text-white rounded shadow-lg">保存</button></div>
            </div></div></div>)}

            {isEventModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4"><div className="absolute inset-0 bg-primary/40 backdrop-blur-sm" onClick={() => setIsEventModalOpen(false)}></div><div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl max-h-[90vh] overflow-y-auto relative animate-enter">
                    <div className="sticky top-0 bg-white/95 backdrop-blur z-20 px-8 py-5 border-b border-gray-100 flex justify-between items-center">
                        <div className="flex flex-col"><h2 className="text-xs font-bold text-gray-400 uppercase tracking-widest">{editingEventId ? 'イベント追加' : 'イベント追加'}</h2><div className="text-xl font-bold text-primary">{eventForm.name || 'イベント名未設定'}</div></div>
                        <div className="flex items-center gap-3">{eventForm.systemId && <a href={`https://andpad.jp/manager/my/orders/${eventForm.systemId}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 px-3 py-1.5 text-red-600 bg-red-50 hover:bg-red-100 border border-red-200 rounded-lg transition-colors font-bold text-xs" title="ANDPAD">ANDPAD <Icon name="external-link" size={11}/></a>}<button onClick={() => setIsEventModalOpen(false)} className="p-2 hover:bg-gray-100 rounded-full transition-colors ml-2"><Icon name="x" size={24}/></button></div>
                    </div>
                    <div className="p-8 space-y-8"><form onSubmit={saveEvent} className="space-y-8">
                        <section className="grid grid-cols-1 md:grid-cols-[120px_1fr] gap-6"><h3 className="text-sm font-bold text-gray-400 uppercase tracking-widest pt-3">基本情報</h3><div className="space-y-4"><div><label className="block text-sm font-bold text-gray-500 mb-1">顧客名</label><div className="flex gap-2"><input type="text" value={eventForm.customerName} onChange={e => setEventForm({...eventForm, customerName: e.target.value})} readOnly={!eventForm.propertyId && !editingEventId} className={`flex-1 px-4 py-2.5 border border-gray-200 rounded-xl outline-none focus:border-accent ${!eventForm.propertyId && !editingEventId ? 'bg-gray-200 text-gray-500 cursor-not-allowed' : 'bg-gray-50'}`} /><button type="button" onClick={()=>openCustomerPickerForPatch('patch_event')} className="px-3 py-2.5 text-blue-600 bg-blue-50 hover:bg-blue-100 border border-blue-200 rounded-xl transition-colors font-bold text-xs whitespace-nowrap flex items-center gap-1" title="顧客を選び直す"><Icon name="search" size={14}/>顧客選択</button></div></div><div><label className="block text-sm font-bold text-gray-500 mb-1">案件名*</label><input type="text" required value={eventForm.name} onChange={e => setEventForm({...eventForm, name: e.target.value})} readOnly={!eventForm.propertyId && !editingEventId} className={`w-full px-4 py-2.5 border border-gray-200 rounded-xl outline-none focus:border-accent ${!eventForm.propertyId && !editingEventId ? 'bg-gray-200 text-gray-500 cursor-not-allowed' : 'bg-gray-50'}`} /></div><div className="grid grid-cols-2 gap-4"><div><label className="block text-sm font-bold text-gray-500 mb-1">住所</label><input type="text" value={eventForm.address} onChange={e => setEventForm({...eventForm, address: e.target.value})} className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-accent" /></div><div><label className="block text-sm font-bold text-gray-500 mb-1">GoogleMapURL</label><div className="flex gap-2"><input type="text" value={eventForm.googleMapUrl} onChange={e => setEventForm({...eventForm, googleMapUrl: e.target.value})} className="flex-1 px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-accent" />{eventForm.googleMapUrl && <a href={eventForm.googleMapUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 px-3 py-2.5 text-blue-600 bg-blue-50 hover:bg-blue-100 border border-blue-200 rounded-xl transition-colors font-bold text-xs whitespace-nowrap" title="Google Map">MAP <Icon name="external-link" size={11}/></a>}</div></div></div><div className="grid grid-cols-2 gap-4"><div><label className="block text-sm font-bold text-gray-500 mb-2">事業部</label><div className="flex flex-wrap gap-2">{CATEGORIES.map(c=><button key={c} type="button" onClick={()=>setEventForm({...eventForm, category:c, subCategory: c==='新築'?eventForm.subCategory:''})} className={`px-4 py-2 rounded-lg text-sm font-bold border ${eventForm.category===c?'bg-accent text-white':'bg-white text-gray-500'}`}>{c}</button>)}</div></div>{eventForm.category==='新築' && <div><label className="block text-sm font-bold text-gray-500 mb-2">ブランド</label><div className="flex flex-wrap gap-2">{['建築設計','LIFIT'].map(s=><button key={s} type="button" onClick={()=>setEventForm({...eventForm,subCategory:eventForm.subCategory===s?'':s})} className={`px-4 py-2 rounded-lg text-sm font-bold border ${eventForm.subCategory===s?'bg-accent text-white':'bg-white text-gray-500'}`}>{s}</button>)}</div></div>}</div></div></section>
                        <section className="grid grid-cols-1 md:grid-cols-[120px_1fr] gap-6 border-t border-gray-100 pt-8"><h3 className="text-sm font-bold text-gray-400 uppercase tracking-widest pt-3">スケジュール</h3><div className="space-y-6"><div><label className="text-sm font-bold text-blue-600 mb-1 block">設営</label><div className="flex gap-2 items-center flex-wrap"><span className="relative inline-block"><input type="date" value={eventForm.setupDate_date} onChange={e=>setEventForm({...eventForm,setupDate_date:e.target.value})} onClick={handleShowPicker} className="w-36 px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-accent text-sm font-bold pr-5"/><DowLabel ymd={eventForm.setupDate_date} className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none bg-gray-50 px-0.5"/></span><select value={eventForm.setupDate_time} onChange={e=>setEventForm({...eventForm,setupDate_time:e.target.value})} className="w-[5.5rem] px-2 py-2.5 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-accent text-sm font-bold"><option value="">開始</option>{timeOptions.map(t=><option key={t} value={t}>{t}</option>)}</select><span className="text-gray-400">→</span><select value={eventForm.setupEndTime} onChange={e=>setEventForm({...eventForm,setupEndTime:e.target.value})} className="w-[5.5rem] px-2 py-2.5 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-accent text-sm font-bold"><option value="">終了</option>{timeOptions.map(t=><option key={t} value={t}>{t}</option>)}</select><select value={eventForm.setupVehicle} onChange={e=>setEventForm({...eventForm,setupVehicle:e.target.value})} className="w-[13.3rem] px-2 py-2.5 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-accent text-sm font-bold"><option value="">車両1</option>{vehicleOptions.map(v=><option key={v.id} value={v.name}>{v.name}</option>)}</select><select value={eventForm.setupVehicle2} onChange={e=>setEventForm({...eventForm,setupVehicle2:e.target.value})} className="w-[13.3rem] px-2 py-2.5 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-accent text-sm font-bold"><option value="">車両2</option>{vehicleOptions.map(v=><option key={v.id} value={v.name}>{v.name}</option>)}</select></div></div><div><label className="text-sm font-bold text-orange-600 mb-1 block">撤収</label><div className="flex gap-2 items-center flex-wrap"><span className="relative inline-block"><input type="date" value={eventForm.teardownDate_date} onChange={e=>setEventForm({...eventForm,teardownDate_date:e.target.value})} onClick={handleShowPicker} className="w-36 px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-accent text-sm font-bold pr-5"/><DowLabel ymd={eventForm.teardownDate_date} className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none bg-gray-50 px-0.5"/></span><select value={eventForm.teardownDate_time} onChange={e=>setEventForm({...eventForm,teardownDate_time:e.target.value})} className="w-[5.5rem] px-2 py-2.5 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-accent text-sm font-bold"><option value="">開始</option>{timeOptions.map(t=><option key={t} value={t}>{t}</option>)}</select><span className="text-gray-400">→</span><select value={eventForm.teardownEndTime} onChange={e=>setEventForm({...eventForm,teardownEndTime:e.target.value})} className="w-[5.5rem] px-2 py-2.5 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-accent text-sm font-bold"><option value="">終了</option>{timeOptions.map(t=><option key={t} value={t}>{t}</option>)}</select><select value={eventForm.teardownVehicle} onChange={e=>setEventForm({...eventForm,teardownVehicle:e.target.value})} className="w-[13.3rem] px-2 py-2.5 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-accent text-sm font-bold"><option value="">車両1</option>{vehicleOptions.map(v=><option key={v.id} value={v.name}>{v.name}</option>)}</select><select value={eventForm.teardownVehicle2} onChange={e=>setEventForm({...eventForm,teardownVehicle2:e.target.value})} className="w-[13.3rem] px-2 py-2.5 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-accent text-sm font-bold"><option value="">車両2</option>{vehicleOptions.map(v=><option key={v.id} value={v.name}>{v.name}</option>)}</select></div></div><div className="border-t border-dashed pt-4"><div className="mb-4"><label className="text-sm font-bold text-purple-600 mb-1 block">イベント名</label><input type="text" value={eventForm.eventName} onChange={e=>setEventForm({...eventForm,eventName:e.target.value})} className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-accent text-sm font-bold" placeholder="例: 完成見学会"/></div><label className="text-sm font-bold text-purple-600 mb-1 block">イベント日</label>{eventForm.eventDates.map((d,i)=>(<div key={i} className="flex gap-2 mt-1"><span className="relative inline-block"><input type="date" value={d} onChange={e=>updateEventDate(i,e.target.value)} onClick={handleShowPicker} className="w-36 px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-accent text-sm font-bold pr-5"/><DowLabel ymd={d} className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none bg-gray-50 px-0.5"/></span>{eventForm.eventDates.length>1&&<button type="button" onClick={()=>removeEventDate(i)} className="text-red-500"><Icon name="trash-2" size={18}/></button>}</div>))}<button type="button" onClick={addEventDate} className="text-sm text-purple-500 mt-1">+ 追加</button></div><div><label className="text-sm font-bold text-emerald-600 mb-1 block">引渡日</label><span className="relative inline-block"><input type="date" value={eventForm.handoverDate} onChange={e=>setEventForm({...eventForm,handoverDate:e.target.value})} onClick={handleShowPicker} className="w-36 px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-accent text-sm font-bold pr-5"/><DowLabel ymd={eventForm.handoverDate} className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none bg-gray-50 px-0.5"/></span></div></div></section>
                        {eventForm.propertyId && (<section className="grid grid-cols-1 md:grid-cols-[120px_1fr] gap-6 border-t border-gray-100 pt-8"><h3 className="text-sm font-bold text-gray-400 uppercase tracking-widest pt-3">撮影依頼</h3><div className="space-y-6"><div><label className="text-sm font-bold text-gray-500 mb-1 block">依頼者</label><select value={eventForm.requester} onChange={e=>setEventForm({...eventForm,requester:e.target.value})} className="w-36 px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-accent text-sm font-bold"><option value="">選択</option>{staffSuggestions.map(s=><option key={s} value={s}>{s}</option>)}</select></div><div><label className="text-sm font-bold text-accent mb-1 block">撮影可能期間</label><div className="flex gap-2 items-center"><span className="relative inline-block"><input type="date" value={eventForm.shootingRange_from_date} onChange={e=>setEventForm({...eventForm,shootingRange_from_date:e.target.value})} onClick={handleShowPicker} className="w-36 px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm font-bold pr-5"/><DowLabel ymd={eventForm.shootingRange_from_date} className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none bg-gray-50 px-0.5"/></span><select value={eventForm.shootingRange_from_time} onChange={e=>setEventForm({...eventForm,shootingRange_from_time:e.target.value})} className="px-2 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm font-bold"><option value="">時間</option>{timeOptions.map(t=><option key={t} value={t}>{t}</option>)}</select><span>~</span><span className="relative inline-block"><input type="date" value={eventForm.shootingRange_to_date} onChange={e=>setEventForm({...eventForm,shootingRange_to_date:e.target.value})} onClick={handleShowPicker} className="w-36 px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm font-bold pr-5"/><DowLabel ymd={eventForm.shootingRange_to_date} className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none bg-gray-50 px-0.5"/></span><select value={eventForm.shootingRange_to_time} onChange={e=>setEventForm({...eventForm,shootingRange_to_time:e.target.value})} className="px-2 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm font-bold"><option value="">時間</option>{timeOptions.map(t=><option key={t} value={t}>{t}</option>)}</select></div></div><div><label className="text-sm font-bold text-gray-500 mb-1 block">撮影種類</label><div className="flex flex-wrap gap-2">{SHOOTING_TYPES.map(t=><button type="button" key={t} onClick={()=>{const n=eventForm.shootingTypes.includes(t)?eventForm.shootingTypes.filter(x=>x!==t):[...eventForm.shootingTypes,t];setEventForm({...eventForm,shootingTypes:n})}} className={`px-3 py-2 border rounded-lg ${eventForm.shootingTypes.includes(t)?'bg-accent text-white':'bg-gray-50'}`}>{t}</button>)}</div></div><div className="grid grid-cols-1 md:grid-cols-2 gap-6"><FileUpload label="撮影指示書 (PDF)" fileUrl={eventForm.instructionFileUrl} fileName={eventForm.instructionFileName} onFileChange={(url, name) => setEventForm({...eventForm, instructionFileUrl: url, instructionFileName: name})} /><FileUpload label="イベント物件概要書 (PDF)" fileUrl={eventForm.overviewFileUrl} fileName={eventForm.overviewFileName} onFileChange={(url, name) => setEventForm({...eventForm, overviewFileUrl: url, overviewFileName: name})} /></div><div className="grid grid-cols-2 gap-4"><div><label className="text-sm font-bold text-gray-500 mb-1 block">立ち合い</label><select value={eventForm.witnessStaff} onChange={e=>setEventForm({...eventForm,witnessStaff:e.target.value})} className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-accent text-sm font-bold"><option value="">選択</option><option value="なし">なし</option>{staffSuggestions.map(s=><option key={s} value={s}>{s}</option>)}</select></div><div><label className="text-sm font-bold text-gray-500 mb-1 block">施主在宅</label><div className="flex gap-2">{['あり','なし'].map(o=><button key={o} type="button" onClick={()=>setEventForm({...eventForm,ownerPresence:o})} className={`flex-1 py-2.5 rounded-xl border text-sm font-bold transition-all ${eventForm.ownerPresence===o?'bg-accent text-white border-accent':'bg-white text-gray-500 border-gray-200'}`}>{o}</button>)}</div></div></div><div><label className="text-sm font-bold text-gray-500 mb-1 block">撮影の注意事項など</label><textarea value={eventForm.shootingNotes} onChange={e=>setEventForm({...eventForm,shootingNotes:e.target.value})} rows={3} className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-accent text-sm" placeholder="撮影時の注意点やメモを入力"/></div></div></section>)}
