                                                ...
                                                if (item._type === 'event') {
                                                    ...
                                                    return (
                                                        <tr key={`evt-${evt.id}`} onDoubleClick={() => openEditModal(evt)} className="hover:bg-teal-50 transition-colors border-b-2 border-gray-300 bg-teal-50/30 cursor-pointer">
                                                            <td className="px-2 py-3 align-top">{(() => { const evtBranch = getAreaBranchDynamic(evt.address, evt.category); const _ed = (evt.eventDates && evt.eventDates.filter(Boolean).length > 0 ? evt.eventDates : (evt.dates || [])).filter(Boolean).map(d => (typeof d === 'string' ? d.split('T')[0] : (d instanceof Date ? d.toISOString().split('T')[0] : String(d).split('T')[0]))); const _today = new Date().toISOString().split('T')[0]; const _statusBadge = _ed.length === 0 ? <span className="px-1.5 py-0.5 rounded text-xs font-bold bg-amber-100 text-amber-700 border border-amber-200">イベント</span> : (() => { const sorted = [..._ed].sort(); const first = sorted[0]; const last = sorted[sorted.length-1]; if (_today > last) return <span className="px-1.5 py-0.5 rounded text-xs font-bold bg-rose-100 text-rose-500 border border-rose-200">イベント終了</span>; if (_today >= first) return <span className="px-1.5 py-0.5 rounded text-xs font-bold bg-red-100 text-red-700 border border-red-200">イベント開催中</span>; return <span className="px-1.5 py-0.5 rounded text-xs font-bold bg-orange-100 text-orange-700 border border-orange-200">イベント予定</span>; })(); return (<div className="flex flex-col gap-1 items-start">{_statusBadge}{(() => { const _ss = computeShootStatus(evt); return _ss ? <span className={`px-1.5 py-0.5 rounded text-xs font-bold border ${_ss.cls}`}>{_ss.label}</span> : null; })()}</div>); })()}</td>
