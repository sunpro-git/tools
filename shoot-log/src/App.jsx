...
const App = () => {
    ...
    const filteredProperties = useMemo(() => {
        return properties.filter(p => {
            ...
            const allDates = [p.setupDate, p.teardownDate, p.photoDate, p.exteriorPhotoDate, p.youtubeDate, p.instaLiveDate, p.instaRegularDate, p.instaPromoDate, p.otherDate, ...(Array.isArray(p.openHouseDates) ? p.openHouseDates : []), ...(Array.isArray(p.eventDates) ? p.eventDates : [])];
            ...
            const hasShooting = !!p.photoDate || !!p.exteriorPhotoDate || !!p.youtubeDate || !!p.instaRegularDate || !!p.instaPromoDate || !!p.instaLiveDate || !!p.otherDate;
    ...
    }, [properties, filterFrom, filterTo, selectedCategories, selectedBranches, selectedScheduleTypes, searchKeyword, viewMode, showNoDateProperties]);
    ...
    const groupedProperties = useMemo(() => {
        ...
        filteredProperties.forEach(p => {
            const hasEvent = p.eventDates && p.eventDates.filter(Boolean).length > 0;
            const hasShooting = (p.shootingTypes && p.shootingTypes.length > 0) || p.youtubeDate || p.photoDate || p.exteriorPhotoDate || p.instaLiveDate || p.instaRegularDate || p.instaPromoDate || p.otherDate;
    ...
    const listenToData = () => {
        ...
        fetchProperties(); fetchStaffs(); fetchEquipments(); fetchSettings();
        ...
        return () => { clearTimeout(propTimer); supabase.removeChannel(ch1); supabase.removeChannel(ch2); supabase.removeChannel(ch3); };
    };
                                        ...
                                        <React.Fragment key={group.title}>
                                            ...
                                            {group.items.map(item => {
                                                if (item._type === 'event') {
                                                    const evt = item;
                                                    const evtSchedule = [
                                                        { label: 'EV設営', date: evt.setupDate, endTime: evt.setupEndTime, color: 'text-indigo-600' },
                                                        { label: 'EV撤収', date: evt.teardownDate, endTime: evt.teardownEndTime, color: 'text-amber-600' },
                                                    ...
                                                    return (
                                                        <tr key={`evt-${evt.id}`} className="hover:bg-teal-50 transition-colors border-b-2 border-gray-300 bg-teal-50/30">
                                                            <td className="px-2 py-3 align-top"><div className="flex flex-col gap-1 items-start"><span className="px-1.5 py-0.5 rounded text-xs font-bold bg-teal-100 text-teal-700 border border-teal-200">イベント</span><span className="px-1.5 py-0.5 rounded text-xs font-bold bg-gray-100 text-gray-600 border border-gray-200">{evt.category}</span></div></td>
                                                            <td className="px-2 py-3 align-top">
                                                                <div className="font-bold text-sm text-gray-900 mb-0.5">{evt.customerName ? <>{evt.customerName}<span className="text-xs font-normal ml-1 text-gray-600">様</span></> : <span className="text-gray-400 font-normal">顧客名未設定</span>}</div>
                                                                <div className="text-xs text-gray-500 font-medium mb-1">{evt.name || '案件名未設定'}</div>
                                                            ...
                                                            <td className="px-2 py-3 align-top">
                                                                <div className="space-y-1">
                                                                    ...
                                                                    {[{l:'YouTube', d:evt.youtubeDate, s:evt.youtubeStartTime, e:evt.youtubeEndTime, r:evt.youtubeRequested},{l:'スチール', d:evt.photoDate, s:evt.photoStartTime, e:evt.photoEndTime, r:evt.photoRequested},{l:'外観撮影', d:evt.exteriorPhotoDate, s:evt.exteriorPhotoStartTime, e:evt.exteriorPhotoEndTime, r:evt.exteriorPhotoRequested},{l:'InstaLive', d:evt.instaLiveDate, s:evt.instaLiveStartTime, e:evt.instaLiveEndTime, r:evt.instaLiveRequested},{l:'インスタ投稿', d:evt.instaRegularDate, s:evt.instaRegularStartTime, e:evt.instaRegularEndTime, r:evt.instaRegularRequested},{l:'インスタ広告', d:evt.instaPromoDate, s:evt.instaPromoStartTime, e:evt.instaPromoEndTime, r:evt.instaPromoRequested},{l:'その他', d:evt.otherDate, s:evt.otherStartTime, e:evt.otherEndTime, r:evt.otherRequested}].filter(x=>x.d).map(m=>(
                                                                        <div key={m.l} className="flex items-center gap-2 text-sm whitespace-nowrap"><span className="w-20 font-bold text-gray-400 text-sm flex items-center gap-0.5">{m.l}{m.r && <Icon name="circle-check" size={14} className="text-green-500 flex-shrink-0"/>}</span><span className="font-normal text-gray-700">{formatDateTime(m.d)}{m.e && <span className="text-[10px] text-gray-400 font-normal"> → <span className="text-[11px]">{m.e}</span></span>}</span></div>
                                                                    ))}
                                                                </div>
                                                            </td>
                                                ...
                                                const hasShooting = !!(prop.youtubeDate || prop.photoDate || prop.exteriorPhotoDate || prop.instaLiveDate || prop.instaRegularDate || prop.instaPromoDate || prop.otherDate);
                                                return (
                                                    <tr key={prop.id} className="hover:bg-yellow-50 transition-colors border-b-2 border-gray-300">
                                                        ...
                                                        <td className="px-2 py-3 align-top">
                                                            <div className="space-y-1">
                                                                ...
                                                                {[{l:'YouTube', d:prop.youtubeDate, s:prop.youtubeStartTime, e:prop.youtubeEndTime, r:prop.youtubeRequested},{l:'スチール', d:prop.photoDate, s:prop.photoStartTime, e:prop.photoEndTime, r:prop.photoRequested},{l:'外観撮影', d:prop.exteriorPhotoDate, s:prop.exteriorPhotoStartTime, e:prop.exteriorPhotoEndTime, r:prop.exteriorPhotoRequested},{l:'InstaLive', d:prop.instaLiveDate, s:prop.instaLiveStartTime, e:prop.instaLiveEndTime, r:prop.instaLiveRequested},{l:'インスタ投稿', d:prop.instaRegularDate, s:prop.instaRegularStartTime, e:prop.instaRegularEndTime, r:prop.instaRegularRequested},{l:'インスタ広告', d:prop.instaPromoDate, s:prop.instaPromoStartTime, e:prop.instaPromoEndTime, r:prop.instaPromoRequested},{l:'その他', d:prop.otherDate, s:prop.otherStartTime, e:prop.otherEndTime, r:prop.otherRequested}].filter(x=>x.d).map(m=>(
                                                                    <div key={m.l} className="flex items-center gap-2 text-sm whitespace-nowrap"><span className="w-20 font-bold text-gray-400 text-sm flex items-center gap-0.5">{m.l}{m.r && <Icon name="circle-check" size={14} className="text-green-500 flex-shrink-0"/>}</span><span className="font-normal text-gray-700">{formatDateTime(m.d)}{m.e && <span className="text-[10px] text-gray-400 font-normal"> → <span className="text-[11px]">{m.e}</span></span>}</span></div>
                                                                ))}
                                                            </div>
                                                        </td>
