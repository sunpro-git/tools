                                            ...
                                            {group.items.map(item => {
                                                if (item._type === 'event') {
                                                    ...
                                                    return (
                                                        <tr key={`evt-${evt.id}`} onDoubleClick={() => openEditModal(evt)} className="hover:bg-teal-50 transition-colors border-b-2 border-gray-300 bg-teal-50/30 cursor-pointer">
                                                            <td className="px-2 py-3 align-top">{(() => { const evtBranch = getAreaBranchDynamic(evt.address, evt.category); const _ed = (evt.eventDates && evt.eventDates.filter(Boolean).length > 0 ? evt.eventDates : (evt.dates || [])).filter(Boolean).map(d => (typeof d === 'string' ? d.split('T')[0] : (d instanceof Date ? d.toISOString().split('T')[0] : String(d).split('T')[0]))); const _today = new Date().toISOString().split('T')[0]; const _statusBadge = _ed.length === 0 ? <span className="px-1.5 py-0.5 rounded text-xs font-bold bg-teal-100 text-teal-700 border border-teal-200">イベント</span> : (() => { const sorted = [..._ed].sort(); const first = sorted[0]; const last = sorted[sorted.length-1]; if (_today > last) return <span className="px-1.5 py-0.5 rounded text-xs font-bold bg-gray-100 text-gray-500 border border-gray-200">イベント終了</span>; if (_today >= first) return <span className="px-1.5 py-0.5 rounded text-xs font-bold bg-emerald-100 text-emerald-700 border border-emerald-200">イベント開催中</span>; return <span className="px-1.5 py-0.5 rounded text-xs font-bold bg-blue-100 text-blue-700 border border-blue-200">イベント予定</span>; })(); return (<div className="flex flex-col gap-1 items-start">{_statusBadge}{(() => { const _ss = computeShootStatus(evt); return _ss ? <span className={`px-1.5 py-0.5 rounded text-xs font-bold border ${_ss.cls}`}>{_ss.label}</span> : null; })()}{evtBranch && <span className={`px-1.5 py-0.5 rounded text-xs font-bold border ${evtBranch.class}`}>{evtBranch.name}</span>}</div>); })()}</td>
                                                            <td className="px-2 py-3 align-top">
                                                                {(() => { const hasCustomer = evt.customerName && evt.customerName !== '顧客なし'; const fallbackTitle = evt.eventName || (evt.name && evt.name !== '案件なし' ? evt.name : ''); return (<div className="mb-1"><div className="font-bold text-sm text-gray-900 flex items-center gap-1.5 flex-wrap">{hasCustomer ? <>{evt.customerName}<span className="text-xs font-normal text-gray-600">様</span></> : (fallbackTitle || <span className="text-gray-400 font-normal">イベント名未設定</span>)}{evt.category && <span className="px-1.5 py-0.5 rounded text-[10px] font-bold bg-gray-100 text-gray-600 border border-gray-200">{evt.category}</span>}{evt.event_type && <span className="px-1.5 py-0.5 rounded text-[10px] font-bold bg-purple-100 text-purple-700 border border-purple-200">{evt.event_type}</span>}</div>{hasCustomer && fallbackTitle && <div className="text-xs text-gray-500 mt-0.5">{fallbackTitle}</div>}</div>); })()}
                                                ...
                                                return (
                                                    <tr key={prop.id} onDoubleClick={() => openEditModal(prop)} className="hover:bg-yellow-50 transition-colors border-b-2 border-gray-300 cursor-pointer">
                                                        <td className="px-2 py-3 align-top"><div className="flex flex-col gap-1 items-start">{(() => { const _ss = computeShootStatus(prop); return _ss ? <span className={`px-1.5 py-0.5 rounded text-xs font-bold border ${_ss.cls}`}>{_ss.label}</span> : null; })()}{branch && <span className={`px-1.5 py-0.5 rounded text-xs font-bold border ${branch.class}`}>{branch.name}</span>}</div></td>
                                                        <td className="px-2 py-3 align-top">
                                                            {(() => { const hasCustomer = prop.customerName && prop.customerName !== '顧客なし'; const fallbackTitle = prop.eventName || (prop.name && prop.name !== '案件なし' ? prop.name : ''); return (<div className="mb-1"><div className="font-bold text-sm text-gray-900 flex items-center gap-1.5 flex-wrap">{hasCustomer ? <>{prop.customerName}<span className="text-xs font-normal text-gray-600">様</span></> : (fallbackTitle || <span className="text-gray-400 font-normal">顧客名未設定</span>)}{prop.category && <span className="px-1.5 py-0.5 rounded text-[10px] font-bold bg-gray-100 text-gray-600 border border-gray-200">{prop.category}</span>}{prop.event_type && <span className="px-1.5 py-0.5 rounded text-[10px] font-bold bg-purple-100 text-purple-700 border border-purple-200">{prop.event_type}</span>}</div>{hasCustomer && fallbackTitle && <div className="text-xs text-gray-500 mt-0.5">{fallbackTitle}</div>}</div>); })()}
