                                                ...
                                                if (item._type === 'event') {
                                                    ...
                                                    return (
                                                        <tr key={`evt-${evt.id}`} className="hover:bg-teal-50 transition-colors border-b-2 border-gray-300 bg-teal-50/30">
                                                            <td className="px-2 py-3 align-top">{(() => { const evtBranch = getAreaBranchDynamic(evt.address, evt.category); return (<div className="flex flex-col gap-1 items-start"><span className="px-1.5 py-0.5 rounded text-xs font-bold bg-teal-100 text-teal-700 border border-teal-200">イベント</span>{((evt.shootingTypes && evt.shootingTypes.length > 0) || evt.shootingRangeFrom || (evt.youtubeDates && evt.youtubeDates.filter(Boolean).length > 0) || evt.youtubeDate || (evt.photoDates && evt.photoDates.filter(Boolean).length > 0) || evt.photoDate || (evt.exteriorPhotoDates && evt.exteriorPhotoDates.filter(Boolean).length > 0) || evt.exteriorPhotoDate || (evt.instaLiveDates && evt.instaLiveDates.filter(Boolean).length > 0) || evt.instaLiveDate || (evt.instaRegularDates && evt.instaRegularDates.filter(Boolean).length > 0) || evt.instaRegularDate || (evt.instaPromoDates && evt.instaPromoDates.filter(Boolean).length > 0) || evt.instaPromoDate || (evt.otherDates && evt.otherDates.filter(Boolean).length > 0) || evt.otherDate) && <span className="px-1.5 py-0.5 rounded text-xs font-bold bg-yellow-100 text-yellow-700 border border-yellow-200">撮影</span>}{evtBranch && <span className={`px-1.5 py-0.5 rounded text-xs font-bold border ${evtBranch.class}`}>{evtBranch.name}</span>}</div>); })()}</td>
