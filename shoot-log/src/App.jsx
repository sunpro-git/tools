...
const App = () => {
    ...
    const [form, setForm] = useState({
        name: '', address: '', category: '新築', subCategory: '', furnitureSetup: 'なし', customerName: '', customerLat: '', customerLon: '', mainStore: '',
                                            ...
                                            {group.items.map(item => {
                                                    ...
                                                    return (
                                                        <tr key={`evt-${evt.id}`} className="hover:bg-teal-50 transition-colors border-b-2 border-gray-300 bg-teal-50/30">
                                                            ...
                                                            <td className="px-2 py-3 align-top">
                                                                {(() => { const hasCustomer = evt.customerName && evt.customerName !== '顧客なし'; const hasName = evt.name && evt.name !== '案件なし'; if (hasCustomer) return <><div className="font-bold text-sm text-gray-900 mb-1.5">{evt.customerName}<span className="text-xs font-normal ml-1 text-gray-600">様</span></div><div className="text-xs text-gray-500 mb-1">{hasName ? evt.name : ''}</div></>; return <div className="font-bold text-sm text-gray-900 mb-1.5">{evt.eventName || <span className="text-gray-400 font-normal">イベント名未設定</span>}</div>; })()}
                                                ...
                                                return (
                                                    <tr key={prop.id} className="hover:bg-yellow-50 transition-colors border-b-2 border-gray-300">
                                                        ...
                                                        <td className="px-2 py-3 align-top">
                                                            <div className="font-bold text-sm text-gray-900 mb-1.5">{prop.customerName ? <>{prop.customerName}<span className="text-xs font-normal ml-1 text-gray-600">様</span></> : <span className="text-gray-400 font-normal">顧客名未設定</span>}</div>
                                                            <div className="text-xs text-gray-500 mb-1">{prop.name}</div>
