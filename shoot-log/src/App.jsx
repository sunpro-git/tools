...
import { formatDate, formatDateTime, toYMD, formatRepName, formatCurrency, handleShowPicker, DowLabel } from './helpers';
                                            ...
                                            {group.items.map(item => {
                                                if (item._type === 'event') {
                                                    ...
                                                    return (
                                                        <tr key={`evt-${evt.id}`} onDoubleClick={() => openEditModal(evt)} className="hover:bg-teal-50 transition-colors border-b-2 border-gray-300 bg-teal-50/30 cursor-pointer">
                                                            <td className="px-2 py-3 align-top">{(() => { const evtBranch = getAreaBranchDynamic(evt.address, evt.category); const _ed = (evt.eventDates && evt.eventDates.filter(Boolean).length > 0 ? evt.eventDates : (evt.dates || [])).filter(Boolean).map(d => (typeof d === 'string' ? d.split('T')[0] : (d instanceof Date ? d.toISOString().split('T')[0] : String(d).split('T')[0]))); const _today = new Date().toISOString().split('T')[0]; const _statusBadge = _ed.length === 0 ? <span className="px-1.5 py-0.5 rounded text-xs font-bold bg-teal-100 text-teal-700 border border-teal-200">イベント</span> : (() => { const sorted = [..._ed].sort(); const first = sorted[0]; const last = sorted[sorted.length-1]; if (_today > last) return <span className="px-1.5 py-0.5 rounded text-xs font-bold bg-gray-100 text-gray-500 border border-gray-200">イベント終了</span>; if (_today >= first) return <span className="px-1.5 py-0.5 rounded text-xs font-bold bg-emerald-100 text-emerald-700 border border-emerald-200">イベント開催中</span>; return <span className="px-1.5 py-0.5 rounded text-xs font-bold bg-blue-100 text-blue-700 border border-blue-200">イベント予定</span>; })(); return (<div className="flex flex-col gap-1 items-start">{_statusBadge}{((evt.shootingTypes && evt.shootingTypes.length > 0) || evt.shootingRangeFrom || (evt.youtubeDates && evt.youtubeDates.filter(Boolean).length > 0) || evt.youtubeDate || (evt.photoDates && evt.photoDates.filter(Boolean).length > 0) || evt.photoDate || (evt.exteriorPhotoDates && evt.exteriorPhotoDates.filter(Boolean).length > 0) || evt.exteriorPhotoDate || (evt.instaLiveDates && evt.instaLiveDates.filter(Boolean).length > 0) || evt.instaLiveDate || (evt.instaRegularDates && evt.instaRegularDates.filter(Boolean).length > 0) || evt.instaRegularDate || (evt.instaPromoDates && evt.instaPromoDates.filter(Boolean).length > 0) || evt.instaPromoDate || (evt.otherDates && evt.otherDates.filter(Boolean).length > 0) || evt.otherDate) && <span className="px-1.5 py-0.5 rounded text-xs font-bold bg-yellow-100 text-yellow-700 border border-yellow-200">撮影</span>}{evtBranch && <span className={`px-1.5 py-0.5 rounded text-xs font-bold border ${evtBranch.class}`}>{evtBranch.name}</span>}</div>); })()}</td>
                                                ...
                                                return (
                                                    <tr key={prop.id} onDoubleClick={() => openEditModal(prop)} className="hover:bg-yellow-50 transition-colors border-b-2 border-gray-300 cursor-pointer">
                                                        <td className="px-2 py-3 align-top"><div className="flex flex-col gap-1 items-start">{hasShooting && <span className="px-1.5 py-0.5 rounded text-xs font-bold bg-amber-100 text-amber-700 border border-amber-200">撮影</span>}{branch && <span className={`px-1.5 py-0.5 rounded text-xs font-bold border ${branch.class}`}>{branch.name}</span>}</div></td>
                                                        <td className="px-2 py-3 align-top">
